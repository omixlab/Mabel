...
def basic():
    ...
    with col3:
        # Boolean operator
        boolean = st.selectbox('Bool', ('AND', 'OR', 'NOT'), key='bool', label_visibility="hidden")
        ...
        if st.button('Add'):
            ...
            if st.session_state.pm_query is None:
                ...
                st.session_state.pm_query += f' {boolean} ({term}{basics.to_pubmed[tag]})'
            ...
            if st.session_state.sc_query is None:
                ...
                st.session_state.sc_query += f' {boolean} {basics.to_scopus[tag]}({term})'

    # Queries
    pm_keyword = st.text_area('PubMed query', st.session_state.pm_query, key='pm_keyword')
    if st.session_state.pm_query is not None:
        st.session_state.pm_query = pm_keyword
    
    sc_keyword = st.text_area('Elsevier query', st.session_state.sc_query, key='sc_keyword')
