    ...
    if st.button("Submit"):
        ...
        if request['pm_check']:
            with st.spinner(f'Searching articles with keyword "{request["pm_keyword"]}" in PubMed ({request["pm_num"]} articles) wait...'):
                data_tmp = extractor.Extractor(request["pm_keyword"], request["pm_num"]).pubmed()
        ...
        if request["sc_check"]:
            with st.spinner(f'Searching articles with keyword "{request["sc_keyword"]}" in Scopus ({request["sc_num"]}) wait...'):
                data_tmp = extractor.Extractor(request["sc_keyword"], request["sc_num"]).scopus()
        ...
        if request["sd_check"]:
            with st.spinner(f'Searching articles with keyword "{request["sd_keyword"]}" in ScienceDirect ({request["sd_num"]}) wait...'):
                data_tmp = extractor.Extractor(request["sd_keyword"], request["sd_num"]).scidir()
        ...
        # UNIFICA AS TABELAS
        st.session_state.dataframes["unified_df"] = unify_dfs.unify(st.session_state.unify_parameters)
